...
def _designation_label(d: str) -> str:
    """
    Convert internal donation designation keys into public-facing labels.

    Legacy internal keys are intentionally retained for compatibility with
    existing frontend forms, stored donations, and API integrations.
    """
    return {
        "general": "General Fund",
        "grace_bridge": "Bright Futures Kenya",
        "education": "Education & Learning",
        "livelihood": "Health & Development",
    }.get(d, "General Fund")
            ...
            line_items = [
                {
                    "price_data": {
                        ...
                        "product_data": {
                            "name": f"NTI Monthly Partnership — {label}",
                        },
                    },
                    "quantity": 1,
                }
...
def verify_webhook(
    ...
    if not settings.STRIPE_WEBHOOK_SECRET:
        # Development convenience: parse without verification.
        log.warning(
            "stripe.webhook.unverified — STRIPE_WEBHOOK_SECRET not set"
        )
...
def handle_event(
    ...
    if et == "checkout.session.completed":
        ...
        log.info(
            "stripe.subscription.deleted id=%s",
            obj.get("id"),
        )

    else:
        log.debug(
            "stripe.event.ignored type=%s",
            et,
        )
...
def _record_checkout_completed(
    ...
    existing = (
        db.query(Donation)
        .filter(Donation.provider_ref == session_id)
        .one_or_none()
    )
    ...
    amount_total = int(
        session_obj.get("amount_total") or 0
    )

    currency = (
        session_obj.get("currency") or "usd"
    ).lower()
    ...
    is_anonymous = str(
        metadata.get("is_anonymous") or "0"
    ) in (
        "1",
        "true",
        "True",
    ...
    if existing:
        ...
        existing.amount_cents = (
            amount_total
            or existing.amount_cents
        )

        existing.donor_email = (
            donor_email
            or existing.donor_email
        )

        existing.donor_name = (
            donor_name
            or existing.donor_name
        )
    ...
    # Send the receipt to the email address supplied by the donor.
    if donor_email:
        try:
            send_donation_receipt(
                ...
                currency=currency,
                frequency=frequency,
                designation=_designation_label(designation),
                reference=session_id,
            )

        except Exception:  # noqa: BLE001
            log.exception(
                "stripe.receipt.send_failed session=%s",
                session_id,
            )


def fetch_and_sync_session(
    db: Session,
    session_id: str,
) -> dict[str, Any]:
    """
    ...
    Used by the /donate/success page to confirm payment without depending
    entirely on webhooks.

    This operation is idempotent and replays through the same handler.
    """
...
def _record_invoice_paid(
    ...
    existing = (
        db.query(Donation)
        .filter(Donation.provider_ref == inv_id)
        .one_or_none()
    )
